...
while(turn<500){
	...
	for (i = 0; i < 18; i++) {
        ...
        fprintf(fp1, "sc%d=%.4f,", i, counter0[i] / smallcompany);
	    fprintf(fp1, "bc%d=%.4f,", i, counter1[i] / bigcompany);
		fprintf(fp1, "str%d=%.4f\t", i, ratio[i]);
	}
    fprintf(fp1, "\n");
	/*
			int count = 0;
	...
	*/
	...
	do {
        ...
        fprintf(fp1, "generation=%d\t,", generation);
		for (i = 0; i < 18; i++) {
			counter[i] = 0;
            counter0[i]=0;
			counter1[i]=0;
